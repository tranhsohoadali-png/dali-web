...
class SettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        $fields = ['tg_token','tg_chat_id','bank_id','bank_acc','bank_name','bank_label','shop_phone','shop_address','ga_id','fb_pixel_id','zalo_link','zalo_oa_id','meta_title','meta_description','meta_keywords','free_ship_from','ship_fee','discount_bank',
            'ghtk_token','ghtk_shop_id','ghtk_pick_address','ghtk_pick_province',
            ...
            'thietke_api_url','thietke_api_key','thietke_test_ips'];
        foreach ($fields as $key) {
            if ($request->has($key)) {
                // updateOrInsert: key mới chưa có dòng trong DB vẫn lưu được
                // (update() thường khớp 0 dòng -> "lưu" im lặng thất bại).
                DB::table('admin_settings')->updateOrInsert(['key' => $key], ['value' => $request->$key, 'updated_at' => now()]);
            }
        }
        // Checkbox bật/tắt VTP (chỉ xử lý khi form VTP được gửi)
        if ($request->has('vtp_form')) {
            DB::table('admin_settings')->updateOrInsert(['key' => 'vtp_enabled'],
                ['value' => $request->boolean('vtp_enabled') ? '1' : '0', 'updated_at' => now()]);
        }
        // Mỗi thẻ cài đặt là 1 form riêng: _section (tên mục) + _anchor (id thẻ để cuộn về)
        $anchor  = preg_replace('/[^a-z0-9\-]/', '', (string) $request->input('_anchor', ''));
        $section = trim((string) $request->input('_section', '')) ?: 'cài đặt';
        $to      = route('admin.settings') . ($anchor ? '#' . $anchor : '');
        if ($request->filled('new_password')) {
            ...
            if ($request->input('new_password') !== $request->input('new_password_confirm')) {
                return redirect($to)->with('error', 'Mật khẩu xác nhận không khớp — KHÔNG đổi mật khẩu.')->withInput();
            }
            DB::table('admin_settings')->updateOrInsert(['key' => 'admin_password'],
                ['value' => Hash::make($request->new_password), 'updated_at' => now()]);
        }
        return redirect($to)->with('success', 'Đã lưu ' . $section . ' thành công!')->with('saved_anchor', $anchor);
    }
}
