...
# polylabelfast là Windows-only compiled extension.
# Thay bằng implementation dùng shapely (cross-platform, chạy trên Ubuntu/Linux).
try:
    from polylabelfast import polylabelfast  # Windows local
except ImportError:
    from shapely.geometry import Polygon as _Polygon

    def polylabelfast(contour, precision: float = 1.0):
        """Drop-in replacement: trả về ([x, y], dist) như polylabelfast."""
        try:
            pts = [(p[0], p[1]) for p in contour]
            poly = _Polygon(pts)
            if not poly.is_valid:
                poly = poly.buffer(0)
            # shapely 2.x có shapely.polylabel; 1.x dùng ops.polylabel
            try:
                from shapely import polylabel as _pl
                pt = _pl(poly, tolerance=precision)
            except ImportError:
                from shapely.ops import polylabel as _pl
                pt = _pl(poly, tolerance=precision)
            dist = float(poly.exterior.distance(pt))
            return ([pt.x, pt.y], dist)
        except Exception:
            # Fallback: centroid
            try:
                c = _Polygon(contour).centroid
                return ([c.x, c.y], 0.0)
            except Exception:
